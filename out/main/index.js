...
function setupAuthHandlers() {
  const db2 = getDb();
  electron.ipcMain.handle("login", (_, { username, password }) => {
    const user = db2.prepare("SELECT * FROM users WHERE username = ? AND active = 1").get(username);
    if (!user) {
    ...
    db2.prepare("UPDATE users SET last_login = CURRENT_TIMESTAMP WHERE id = ?").run(user.id);
  ...
  electron.ipcMain.handle("get-users", () => {
    return db2.prepare("SELECT id, username, full_name, role, email, active, last_login, created_at FROM users ORDER BY full_name").all();
  });
  electron.ipcMain.handle("add-user", (_, { username, password, full_name, role, email }) => {
  ...
  electron.ipcMain.handle("update-user", (_, { id, full_name, role, email, active }) => {
    db2.prepare(`
  ...
  electron.ipcMain.handle("change-password", (_, { id, currentPassword, newPassword }) => {
    const user = db2.prepare("SELECT password_hash FROM users WHERE id = ?").get(id);
    if (!user || !verifyPassword(currentPassword, user.password_hash)) {
      return { success: false, error: "Contraseña actual incorrecta" };
    }
    db2.prepare("UPDATE users SET password_hash = ? WHERE id = ?").run(hashPassword(newPassword), id);
    logAction(id, "CHANGE_PASSWORD", "AUTH", `Contraseña cambiada para usuario ID: ${id}`);
    return { success: true };
  });
  electron.ipcMain.handle("get-audit-logs", (_, { limit, offset } = { limit: 100, offset: 0 }) => {
    const logs = db2.prepare(`
      ...
      LIMIT ? OFFSET ?
    `).all(limit, offset);
    const total = db2.prepare("SELECT COUNT(*) as count FROM audit_logs").get();
    return { logs, total: total.count };
  });
  electron.ipcMain.handle("get-staff", () => {
    return db2.prepare(`
  ...
  electron.ipcMain.handle("add-staff", (_, { userId, specialty, licenseNumber, phone }) => {
    const info = db2.prepare(`
  ...
  electron.ipcMain.handle("update-staff", (_, { id, specialty, licenseNumber, phone, active }) => {
    db2.prepare(`
  ...
  electron.ipcMain.handle("delete-patient", (_, id) => {
    db2.prepare("DELETE FROM patient_images WHERE patient_id = ?").run(id);
    db2.prepare("DELETE FROM clinical_notes WHERE patient_id = ?").run(id);
    db2.prepare("DELETE FROM prescriptions WHERE patient_id = ?").run(id);
    db2.prepare("DELETE FROM odontograms WHERE patient_id = ?").run(id);
    db2.prepare("DELETE FROM appointments WHERE patient_id = ?").run(id);
    db2.prepare("DELETE FROM invoices WHERE patient_id = ?").run(id);
    const result = db2.prepare("DELETE FROM patients WHERE id = ?").run(id);
